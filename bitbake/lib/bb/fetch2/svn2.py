...
class Svn2(FetchMethod):
    """Class to fetch a module or modules from svn repositories"""
    def supports(self, ud, d):
        """
        Check to see if a given url can be fetched with svn.
        """
        return ud.type in ['svn2']
    ...
    def urldata_init(self, ud, d):
        ...
        # hack: replace directory name to make it look more like a autoconf tarball with correct revision
        if not "path_spec" in ud.parm:
            # hack: the revision number is not coming from pv so append manually
            pv = d.getVar("PV", False)
            ud.path_spec = "%s-%s%s" % (ud.module, pv, ud.revision)
    ...
    def _buildsvncommand(self, ud, d, command):
        ...
        if command == "info":
            svncmd = "%s info %s %s://%s/" % (ud.basecmd, " ".join(options), proto, svnroot)
        elif command == "log1":
            svncmd = "%s log --limit 1 %s %s://%s/" % (ud.basecmd, " ".join(options), proto, svnroot)
        else:
            suffix = ""
            if ud.revision:
            ...
            if command == "fetch":
                ...
                svncmd = "%s co %s %s://%s%s%s %s" % (ud.basecmd, " ".join(options), proto, transportuser, svnroot, suffix, ud.path_spec)
            elif command == "update":
